...
import pytest
from wintermute.WintermuteConsole import WintermuteConsole, BuilderContext, get_visible_state
from wintermute.core import Operation, Device, User, AWSAccount, Service
...
def test_cmd_status_no_op(console: WintermuteConsole, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_cmd_operation_save_failure(console: WintermuteConsole, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_cmd_operation_load_failure(console: WintermuteConsole, capsys: pytest.CaptureFixture[str]) -> None:
    mock_backend = MagicMock()
...
def test_cmd_operation_load_success(console: WintermuteConsole, capsys: pytest.CaptureFixture[str]) -> None:
    mock_backend = MagicMock()
...
def test_cmd_operation_delete_success(console: WintermuteConsole, capsys: pytest.CaptureFixture[str]) -> None:
    mock_backend = MagicMock()
...
def test_cmd_operation_delete_failure(console: WintermuteConsole, capsys: pytest.CaptureFixture[str]) -> None:
    mock_backend = MagicMock()
...
def test_cmd_operation_delete_no_support(console: WintermuteConsole, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_cmd_edit_failure(console: WintermuteConsole, capsys: pytest.CaptureFixture[str]) -> None:
    console.cmd_edit("nonexistent")
    captured = capsys.readouterr()
    assert "Could not find object" in captured.out

def test_cmd_set_no_cartridge(console: WintermuteConsole, capsys: pytest.CaptureFixture[str]) -> None:
    console.cmd_set("opt", "val")
    captured = capsys.readouterr()
    assert "No cartridge selected" in captured.out

def test_cmd_run_no_cartridge(console: WintermuteConsole, capsys: pytest.CaptureFixture[str]) -> None:
    console.cmd_run()
    captured = capsys.readouterr()
    assert "No cartridge selected" in captured.out

def test_show_options_no_cartridge(console: WintermuteConsole, capsys: pytest.CaptureFixture[str]) -> None:
...
async def test_cmd_ai_chat(console: WintermuteConsole) -> None:
    ...
    with patch("wintermute.WintermuteConsole.tool_calling_chat", return_value=mock_resp):
        await console.cmd_ai("chat", "hi")

@pytest.mark.asyncio
async def test_cmd_backend_setup_flow(console: WintermuteConsole) -> None:
    with patch.object(console, "_get_backend_params", return_value=[("path", "str")]):
        console.session.prompt_async = MagicMock(return_value=asyncio.Future())
        console.session.prompt_async.return_value.set_result("/tmp")
        
        mock_mod = MagicMock()
        mock_mod.__category__ = "Storage"
        mock_cls = MagicMock()
        with patch("importlib.import_module", return_value=mock_mod):
            with patch("inspect.getmembers", return_value=[("JsonFileBackend", mock_cls)]):
                await console.cmd_backend_setup("json_file_backend")
...
def test_cmd_builder_save_new_service_root(console: WintermuteConsole, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_cmd_builder_save_unsupported_root(console: WintermuteConsole, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_cmd_delete_success(console: WintermuteConsole, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_cmd_delete_cancel(console: WintermuteConsole, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_cmd_delete_user(console: WintermuteConsole, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_cmd_delete_aws(console: WintermuteConsole, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_cmd_delete_peripheral(console: WintermuteConsole, monkeypatch: pytest.MonkeyPatch) -> None:
    console.operation.addDevice("host1")
...
def test_cmd_delete_vulnerability(console: WintermuteConsole, monkeypatch: pytest.MonkeyPatch) -> None:
    console.operation.addDevice("host1")
    dev = console.operation.getDeviceByHostname("host1")
    assert dev is not None
